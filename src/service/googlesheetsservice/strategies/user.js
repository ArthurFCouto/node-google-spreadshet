...
class userStrategies extends customInterface {
  ...
  async getAll() {
    try {
      ...
      return modelResponseError('Ops! Erro ao buscar usuários cadastrados', error);
    }
  }
  ...
  async create(data) {
    try {
      ...
      return modelResponseError('Ops! Erro durante o cadastro do usuário', error);
    }
  }
  ...
  async getById(id) {
    try {
      ...
      return modelResponseError('Ops! Erro durante a pesquisa pelo email', error);
    }
  }
    ...
    try {
      if (id && data) {
        ...
        if (user) {
          ...
          return modelResponseError('Ops! Não foi possível concluir a atualização', { ...customError[400], data: 'Enviar o nome, telefone e imagem corretamente' });
        }
        return modelResponseError(`Ops! O usuário com email ${id} não está cadastrado`, customError[404]);
      }
      return modelResponseError('Ops! Erro ao atualizar o usuário', { ...customError[400], data: 'Enviar o email e os dados corretamente' });
    } catch (error) {
      return modelResponseError('Ops! Erro durante a atualização do usuário', error);
    }
  }
  ...
  async delete(id) {
    try {
      ...
      return modelResponseError('Ops! Erro durante a exclusão do usuário', error);
    }
  }
}
